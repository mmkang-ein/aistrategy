...
if exist "%~dp0app.py" if exist "%~dp0requirements.txt" (
    echo  [INFO] 기존 설치 감지 - 최신 코드로 업데이트 중...
    git pull --autostash
    if %errorlevel%==0 (
        echo  [OK] 코드 업데이트 완료
    ) else (
...
if %errorlevel% neq 0 (
    echo.
    echo  [오류] 다운로드 실패 - 인터넷 연결 및 GitHub 접근 권한을 확인하세요.
    echo.
    pause
    exit
)
...
echo.
echo  다음번부터는 아래 파일을 더블클릭하면 실행됩니다:
if not defined APP_DIR set APP_DIR=%~dp0
